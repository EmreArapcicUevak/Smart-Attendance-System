...
@Observable
class StudentDashboardViewController {
    ...
    func fetchCourses() async throws {
        ...
        guard let identityModel = SessionExpirationManager.shared.identityModel else { throw loadingErrors.invalidToken }
    ...
    func loadView() async {
        ...
        do {
            ...
            SessionExpirationManager.shared.errorAndNotificationController?.displayErrorMessage("Your token isn't valid anymore.")
            ...
            self.errorAndNotificationController.displayErrorMessage("Couldn't get a response from the server.")
        } catch(loadingErrors.noData) {
            self.errorAndNotificationController.displayPopUpMessage("You current aren't assigned into any courses.")
        } catch {
